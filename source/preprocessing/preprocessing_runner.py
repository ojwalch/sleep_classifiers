...
def run_preprocessing(subject_set):
    ...
    if Constants.INCLUDE_CIRCADIAN:
        ActivityCountService.build_activity_counts()  # This uses MATLAB,
        # but has been replaced with a python implementation
        CircadianService.build_circadian_model()  # Both of the circadian
        # lines require MATLAB to run
        CircadianService.build_circadian_mesa()  # INCLUDE_CIRCADIAN = False
        # by default because most people don't have MATLAB
...
def run_preprocessing_disordered_sleepers(subject_set):
    start_time = time.time()

    for subject in subject_set:
        print("Cropping data from subject " + str(subject) + "...")
        RawDataProcessor.crop_all_disordered(str(subject))
...
disordered_subject_ids = SubjectBuilder.get_all_disordered_subject_ids()
run_preprocessing_disordered_sleepers(disordered_subject_ids)

for subject_id in disordered_subject_ids:
    DataPlotBuilder.make_data_demo(subject_id, False)
