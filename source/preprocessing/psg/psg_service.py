...
from source.preprocessing.psg.psg_raw_data_collection import \
    PSGRawDataCollection
...
class PSGService(object):
    ...
    def get_path_to_file(subject_id):
        ...
        compumedics_file = psg_dir.joinpath(
            'compumedics/AW0' + subject_id.zfill(2) + '.TXT')
        if compumedics_file.is_file():
            return compumedics_file

        txt_file = psg_dir.joinpath(
            'vitaport/AW0' + subject_id.zfill(2) + '011.txt')
        if txt_file.is_file():
            return txt_file
    ...
    def get_type_and_report(subject_id):
        ...
        pdf_file = report_dir.joinpath(
            'AW0' + subject_id.zfill(2) + '011_REPORT.pdf')
        if pdf_file.is_file():
            return pdf_file, PSGFileType.Compumedics

        docx_file = report_dir.joinpath(
            'AW00' + subject_id + '011 Study Sleep Log.docx')
    ...
    def read_raw(subject_id):
        ...
        if psg_type == PSGFileType.Compumedics:
            report_summary = PSGReportProcessor.get_summary_from_pdf(
                psg_report_path)
            report_summary.start_epoch = \
                PSGReportProcessor.get_start_epoch_for_subject(
                subject_id)
        ...
        if psg_type == PSGFileType.Vitaport:
            report_summary = PSGReportProcessor.get_summary_from_docx(
                psg_report_path)
    ...
    def read_precleaned(subject_id):
        psg_path = str(utils.get_project_root().joinpath(
            'data/labels/' + subject_id + '_labeled_sleep.txt'))
        data = []
    ...
    @staticmethod
    def read_precleaned_disordered(subject_id):
        data = []
    ...
    def write(psg_raw_data_collection):
        ...
        for index in range(len(psg_raw_data_collection.data)):
            ...
            data_array.append(
                [stage_item.epoch.timestamp, stage_item.stage.value])

        np_psg_array = np.array(data_array)
        psg_output_path = Constants.CROPPED_FILE_PATH.joinpath(
            psg_raw_data_collection.subject_id + "_cleaned_psg.out")
    ...
    def load_cropped(subject_id):
        ...
        for row in range(np.shape(cropped_array)[0]):
            ...
            stage_items.append(StageItem(
                epoch=Epoch(timestamp=cropped_array[row, 0], index=row),
